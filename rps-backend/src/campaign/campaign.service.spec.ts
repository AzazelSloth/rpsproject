...
describe('CampaignService', () => {
  ...
  it('retries with /webhook/rps-aanalysis when /webhook/rps-analysis returns 404', async () => {
    process.env.N8N_WEBHOOK_URL = 'http://n8n.test/webhook/rps-analysis';
    (service as any).n8nWebhookUrl = 'http://n8n.test/webhook/rps-analysis';
    jest
      .spyOn(service as any, 'getCampaignResponsesFormatted')
      .mockResolvedValue([{ Employeur: 'Entreprise Test', Q1: '4' }]);
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      'http://n8n.test/webhook/rps-analysis',
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      'http://n8n.test/webhook/rps-aanalysis',
      expect.objectContaining({ method: 'POST' }),
    );
  });
});
