...
SURVEY_EXPORT_PSEUDONYM_SECRET="${SURVEY_EXPORT_PSEUDONYM_SECRET:-}"
...
require_non_empty API_KEY

if [ -n "$TEST_SURVEY_DELETE_ALLOWED_EMAILS" ]; then
  if [ "${#SURVEY_EXPORT_PSEUDONYM_SECRET}" -lt 32 ]; then
    echo "ERROR: Survey exports require SURVEY_EXPORT_PSEUDONYM_SECRET with at least 32 characters."
    exit 1
  fi
fi
