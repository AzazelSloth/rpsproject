...
TEST_SURVEY_DELETE_ALLOWED_EMAILS="${TEST_SURVEY_DELETE_ALLOWED_EMAILS:-}"
...
N8N_WEBHOOK_PATH="${N8N_WEBHOOK_PATH:-/webhook/rps-analysis}"
...
BACKEND_MEMORY_RESERVATION="${BACKEND_MEMORY_RESERVATION:-}"
BACKEND_CPU_LIMIT="${BACKEND_CPU_LIMIT:-}"
BACKEND_PIDS_LIMIT="${BACKEND_PIDS_LIMIT:-}"
BACKEND_NODE_OPTIONS="${BACKEND_NODE_OPTIONS:-}"
FRONTEND_MEMORY_LIMIT="${FRONTEND_MEMORY_LIMIT:-}"
FRONTEND_MEMORY_RESERVATION="${FRONTEND_MEMORY_RESERVATION:-}"
FRONTEND_CPU_LIMIT="${FRONTEND_CPU_LIMIT:-}"
FRONTEND_PIDS_LIMIT="${FRONTEND_PIDS_LIMIT:-}"
FRONTEND_NODE_OPTIONS="${FRONTEND_NODE_OPTIONS:-}"
NGINX_MEMORY_LIMIT="${NGINX_MEMORY_LIMIT:-}"
NGINX_MEMORY_RESERVATION="${NGINX_MEMORY_RESERVATION:-}"
NGINX_CPU_LIMIT="${NGINX_CPU_LIMIT:-}"
NGINX_PIDS_LIMIT="${NGINX_PIDS_LIMIT:-}"
N8N_MEMORY_LIMIT="${N8N_MEMORY_LIMIT:-}"
N8N_MEMORY_RESERVATION="${N8N_MEMORY_RESERVATION:-}"
N8N_CPU_LIMIT="${N8N_CPU_LIMIT:-}"
N8N_PIDS_LIMIT="${N8N_PIDS_LIMIT:-}"
N8N_NODE_OPTIONS="${N8N_NODE_OPTIONS:-}"
...
if [ -z "$N8N_SECURE_COOKIE" ]; then
  case "$N8N_EDITOR_BASE_URL:$WEBHOOK_URL:$PUBLIC_BASE_URL" in
    https://*|*:https://*|*:*:https://*)
...
if [ -z "$N8N_WEBHOOK_URL" ]; then
  N8N_WEBHOOK_URL="$(trim_trailing_slash "$WEBHOOK_URL")/${N8N_WEBHOOK_PATH#/}"
fi

SENDGRID_CREDENTIALS_OVERWRITE_DATA=""
if [ -n "$SENDGRID_API_KEY" ]; then
  escaped_sendgrid_api_key="$(json_escape "$SENDGRID_API_KEY")"
  SENDGRID_CREDENTIALS_OVERWRITE_DATA="{\"sendGridApi\":{\"apiKey\":\"$escaped_sendgrid_api_key\"}}"
fi
...
require_non_empty BACKEND_MEMORY_LIMIT
require_non_empty BACKEND_MEMORY_RESERVATION
require_non_empty BACKEND_CPU_LIMIT
require_non_empty BACKEND_PIDS_LIMIT
require_non_empty BACKEND_NODE_OPTIONS
require_non_empty FRONTEND_MEMORY_LIMIT
require_non_empty FRONTEND_MEMORY_RESERVATION
require_non_empty FRONTEND_CPU_LIMIT
require_non_empty FRONTEND_PIDS_LIMIT
require_non_empty FRONTEND_NODE_OPTIONS
require_non_empty NGINX_MEMORY_LIMIT
require_non_empty NGINX_MEMORY_RESERVATION
require_non_empty NGINX_CPU_LIMIT
require_non_empty NGINX_PIDS_LIMIT
require_non_empty N8N_MEMORY_LIMIT
require_non_empty N8N_MEMORY_RESERVATION
require_non_empty N8N_CPU_LIMIT
require_non_empty N8N_PIDS_LIMIT
require_non_empty N8N_NODE_OPTIONS
...
CREDENTIALS_OVERWRITE_DATA=$SENDGRID_CREDENTIALS_OVERWRITE_DATA
N8N_MEMORY_LIMIT=$N8N_MEMORY_LIMIT
N8N_MEMORY_RESERVATION=$N8N_MEMORY_RESERVATION
N8N_CPU_LIMIT=$N8N_CPU_LIMIT
N8N_PIDS_LIMIT=$N8N_PIDS_LIMIT
